...
public:
    ...
    ProblemData(Matrix const* conflictMatrix, AdjacencyList const* graph,
                ...
                int numEnrolments, boost::unordered_map<int, int> const* courseStudentCounts,
                vector<string> const* courseNames)
    ...
    boost::unordered_map<int, int> const* getCourseStudentCounts() const { return courseStudentCounts; }
