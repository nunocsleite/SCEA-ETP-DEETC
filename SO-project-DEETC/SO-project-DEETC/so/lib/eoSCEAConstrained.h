#ifndef EOSCEACONSTRAINED_H
#define EOSCEACONSTRAINED_H
...
*/
template <class EOT>
class eoSCEAConstrained : public eoAlgo<EOT>
{
public:
     ...
     */
    eoSCEAConstrained<EOT>(string const& _outputDir, ofstream& _outFile, string const& _filename,
...
//                if (rng.uniform() < 0.2) { // LAST
                    vector<boost::unordered_set<int> >& pwPeriods = _Pw.getPeriods();
                    // Get newPw periods
                    vector<boost::unordered_set<int> >& newPwPeriods = newPw.getPeriods();
...
//                    int randNumPeriods = 2;
                    ...
                    for (int i = 1; i <= randNumPeriods; ++i) {
                        ...
                        boost::unordered_set<int>& pwExams = pwPeriods[randPwPeriod];

                        // newPw exams from rand period
                        boost::unordered_set<int>& newPwExams = newPwPeriods[randNewPwPeriod];

                        // Insert Pw period into newPw into that random position
                        for (auto it = pwExams.begin(); it != pwExams.end(); ++it) {

                            ///
                            /// CHANGED IN ORDER TO SUPPORT H2 CONSTRAINT IN TWO/EPOCH PROBLEM
                            ///
...
//                                newPw.isFeasiblePeriodExam(randNewPwPeriod, *it)) {
                            if (newPwExams.find(*it) == newPwExams.end() &&
                                randNewPwPeriod <= firstEpochPeriodsUpperBound[*it] && /// ADDED
                                newPw.isFeasiblePeriodExam(randNewPwPeriod, *it)) {
        ...
        //                        cin.get();
                ...
                //                        if (i != bestDayIdx) {
                                        boost::unordered_set<int>& exams = newPwPeriods[i];
...
//                if (rng.uniform() < 0.05) {
                if (rng.uniform() < mp) {
                    ...
                    //
                    // Get Frog1 periods
                    vector<boost::unordered_set<int> >& chromPeriods = _chromosome.getPeriods();
...
//                    moGDA<ETTPneighborProximityCost> gda(neighborhood, fullEval, neighEval, up);

                    ///
                    /// CHANGED IN ORDER TO SUPPORT H2 CONSTRAINT IN TWO/EPOCH PROBLEM
                    ///
                    ETTPneighborhood<ETTPneighborProximityCost<EOT> > neighborhood(&firstEpochPeriodsUpperBound); /// CHANGED
...
protected:
    ...
    // Population variance
    double popVariance;
    // 1st epoch periods upper bound
    vector<int> const& firstEpochPeriodsUpperBound;
};


#endif // EOSCEACONSTRAINED_H
