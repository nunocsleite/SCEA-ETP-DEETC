...
void GCHeuristics::SD(Matrix const& conflictMatrix, AdjacencyList const& graph, Chromosome& timetable,
                      vector<int> const& fixedExams, boost::unordered_map<int, int> const& examsPeriods,
    ...
    // Timetable's periods
    vector<boost::unordered_set<int> >& periods = timetable.getPeriods();
...
//        exams.push_back(ei);
    ...
    vector<boost::unordered_set<int> > examsAvailablePeriodsList(N+1);
    boost::unordered_set<int> allPeriods;
...
//            tj = availablePeriodsList[index-1];
            boost::unordered_set<int>& availablePeriodsList = examsAvailablePeriodsList[ei];
...
//            cin.get();
            ...
            boost::unordered_set<int> newPeriod;
            newPeriod.insert(ei);
...
//            cout << "ej = " << ej << endl;
            ...
            if (!removed_exams[ej])  {
                ...
                boost::unordered_set<int>& remainingPeriods = examsAvailablePeriodsList[ej];
...
void GCHeuristics::insertFixedExams(vector<boost::unordered_set<int> >& periods, vector<bool>& removed_exams, int numPeriods,
                                    VertexPriorityQueue& pq, vector<boost::unordered_set<int> >& examsAvailablePeriodsList,
                                    vector<int> const& fixedExams, boost::unordered_map<int, int> const& examsPeriods,
